        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if step % gradient_accumulation_steps == 0:
                    ...
                    step_io_drive_prefetch_acc = 0.0
                    step_io_drive_staged_read_acc = 0.0
                    step_io_prefetch_job_wall_acc = 0.0
                ...
                if isinstance(io_stats, dict):
                    ...
                    step_io_drive_prefetch_acc += float(io_stats.get("drive_prefetch_batch_s", 0.0))
                    step_io_prefetch_job_wall_acc += float(io_stats.get("prefetch_job_wall_s", 0.0))
                    ...
                    step_io_drive_staged_read_acc += float(io_stats.get("drive_staged_np_load_s", 0.0))
                ...
                if train_config.use_fp16:
                    ...
                    if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                        ...
                        tqdm.write(
                            ...
                            f"IO明细 Job:{step_io_prefetch_job_wall_acc:.3f}s gcs↓:{step_io_gcs_dl_acc:.3f}s "
                            f"drive预取:{step_io_drive_prefetch_acc:.3f}s gcs读:{step_io_gcs_read_acc:.3f}s "
                            f"drv读:{step_io_drive_staged_read_acc:.3f}s "
                            f"drive拷LRU:{step_io_drive_copy_acc:.3f}s drive直读:{step_io_drive_mount_acc:.3f}s "
